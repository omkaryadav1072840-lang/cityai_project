...
router.get("/api/admin/command-center", authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const [
            ...
            pool.query("SELECT COUNT(*) AS total, SUM(CASE WHEN status IN ('Pending', 'Submitted', 'In Progress') THEN 1 ELSE 0 END) AS pending FROM waste_bin_requests"),
            pool.query("SELECT IFNULL(SUM(total_beds), 2000) AS total, IFNULL(SUM(total_beds) - IFNULL((SELECT COUNT(*) FROM hospital_ward_beds WHERE status = 'Occupied'), 350), 1650) AS available, IFNULL(SUM(icu_beds), 370) AS icu_available FROM hospitals"),
            pool.query("SELECT COUNT(*) AS total, SUM(CASE WHEN status = 'Available' THEN 1 ELSE 0 END) AS available FROM ambulances"),
            pool.query("SELECT COUNT(*) AS active FROM emergency_incidents WHERE status IN ('ACTIVE', 'Active', 'Reported', 'Dispatched', 'En Route')"),
